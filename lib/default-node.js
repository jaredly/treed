...
DefaultNode.prototype = {
  ...
  insertReturn: function () {
    var pos = this.getSelectionPosition()
      ...
      , next = text.slice(0, pos) + '\n' + text.slice(pos)
    if (pos === text.length) {
      next += '\n'
      pos += 1
    }
    console.log([text, next], pos)
    
    this.input.innerHTML = next
    this.input.focus()
    var sel = window.getSelection()
      , ran = document.createRange()
    ran.setStart(this.input.firstChild, pos + 1)
    sel.removeAllRanges()
    sel.addRange(ran)
  },
  getSelectionPosition: function () {
    var sel = window.getSelection()
    console.log(sel.getRangeAt(0), sel.baseOffset, sel, this.input.childNodes)
